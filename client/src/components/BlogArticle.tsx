...
export default function BlogArticle() {
  const [, paramsEs] = useRoute("/blog/:slug");
    const [, paramsEn] = useRoute("/en/blog/:slug");
    const params = paramsEn || paramsEs;
  ...
  if (!article) return <main className="grid min-h-screen place-items-center bg-[#f6fafc] p-6 text-center"><div><h1 className="text-3xl font-bold text-[#1A2B3C]">Artículo no encontrado</h1><Link href={`${prefix}/blog`} className="mt-4 inline-block font-bold text-[#007e88]">Volver al blog</Link></div></main>;
  ...
  return <main className="min-h-screen bg-white pb-20">
    ...
    <div className="container">
      ...
      <section className="mx-auto max-w-6xl border-t border-[#dcebf3] py-12"><h2 className="text-2xl font-bold text-[#1A2B3C]">Artículos relacionados</h2><div className="mt-6 grid gap-5 md:grid-cols-3">{related.map((item) => <Link key={item.slug} href={`${prefix}/blog/${item.slug}`} className="overflow-hidden rounded-2xl border border-[#dcebf3] bg-white transition hover:shadow-lg"><img src={item.image} alt={item.imageAlt} width="500" height="300" loading="lazy" className="h-36 w-full object-cover" /><div className="p-4"><span className="text-xs font-bold text-[#00B8C8]">{item.category}</span><h3 className="mt-2 font-bold leading-snug text-[#1A2B3C]">{item.title}</h3></div></Link>)}</div><div className="mt-8 grid gap-3 sm:grid-cols-2"><Link href={`${prefix}/blog/${previous.slug}`} className="rounded-2xl bg-[#f6fafc] p-5 text-sm font-bold text-[#003F7D]">← {previous.title}</Link><Link href={`${prefix}/blog/${next.slug}`} className="rounded-2xl bg-[#f6fafc] p-5 text-right text-sm font-bold text-[#003F7D]">{next.title} →</Link></div></section>
