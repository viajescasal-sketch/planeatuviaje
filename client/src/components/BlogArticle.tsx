...
export default function BlogArticle() {
  ...
  useEffect(() => {
    ...
    const canonicalUrl = `${window.location.origin}/planeatuviaje/blog/${article.slug}`;
  ...
  const shareUrl = encodeURIComponent(`${window.location.origin}/planeatuviaje/blog/${article.slug}`);

  return <main className="min-h-screen bg-white pb-20">
    <header className="bg-[#003F7D] px-4 pb-12 pt-8 text-white"><div className="container"><div className="flex flex-wrap items-center justify-between gap-3"><Link href="/blog" className="text-sm font-bold text-white/80 hover:text-white">← Volver al blog</Link><a href="/planeatuviaje/" className="text-sm font-bold text-white/80 hover:text-white">Viajes Casal</a></div><div className="mx-auto mt-12 max-w-4xl text-center"><span className="rounded-full bg-white/10 px-4 py-2 text-xs font-bold text-[#9ef4ef]">{article.category}</span><h1 className="mt-6 text-3xl font-extrabold leading-tight sm:text-5xl">{article.title}</h1><div className="mt-5 flex justify-center gap-4 text-sm text-white/70"><span>{article.date}</span><span>•</span><span>{article.readingTime} de lectura</span></div></div></div></header>
    <div className="container -mt-1"><img src={article.image} alt={article.imageAlt} width="1400" height="800" fetchPriority="high" className="mx-auto h-[320px] w-full max-w-5xl rounded-b-3xl object-cover shadow-xl sm:h-[520px]" />
