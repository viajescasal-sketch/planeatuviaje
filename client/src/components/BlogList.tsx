...
export default function BlogList() {
  ...
  return <main className="min-h-screen bg-[#f6fafc] pb-20">
    <header className="relative isolate overflow-hidden px-4 pb-28 pt-16 text-white" style={{ backgroundImage: "url(https://images.unsplash.com/photo-1488646953014-85cb44e25828?auto=format&fit=crop&w=1800&q=75&fm=webp)", backgroundPosition: "center", backgroundSize: "cover" }}>
      <div className="absolute inset-0 -z-10 bg-gradient-to-r from-[#002c59]/95 via-[#003F7D]/82 to-[#00B8C8]/45" /><div className="container"><a href="/planeatuviaje/" className="text-sm font-semibold text-white/85 hover:text-white">← Volver a Viajes Casal</a><div className="mt-16 max-w-3xl"><p className="text-xs font-bold uppercase tracking-[0.2em] text-[#b8fffb]">Blog Viajes Casal</p><h1 className="mt-4 text-4xl font-extrabold leading-tight drop-shadow-lg sm:text-6xl" style={{ color: "#FFFFFF" }}>Viaja mejor desde la planeación</h1><p className="mt-5 max-w-2xl text-base leading-relaxed text-white/90 sm:text-lg">Guías claras, comparaciones honestas y consejos prácticos para elegir destinos, cuidar tu presupuesto y disfrutar cada etapa del viaje.</p></div>
        <div className="mt-9 max-w-2xl"><label htmlFor="blog-search" className="sr-only">Buscar artículos</label><div className="flex items-center rounded-2xl bg-white p-2 shadow-xl"><svg className="ml-3 text-[#5a7080]" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg><input id="blog-search" value={query} onChange={(e) => setQuery(e.target.value)} className="min-w-0 flex-1 px-3 py-2 text-sm text-[#1A2B3C] outline-none" placeholder="Buscar destinos, playas o consejos..." /></div></div>
      </div>
    </header>
    <div className="container -mt-8">
      <div className="flex gap-2 overflow-x-auto rounded-2xl bg-white p-3 shadow-lg" aria-label="Filtrar artículos">{blogCategories.map((item) => <button key={item} onClick={() => setCategory(item)} className={`whitespace-nowrap rounded-full px-4 py-2 text-xs font-bold transition ${category === item ? "bg-[#00B8C8] text-white" : "bg-[#eef6fa] text-[#496575] hover:bg-[#dff3f5]"}`}>{item}</button>)}</div>
